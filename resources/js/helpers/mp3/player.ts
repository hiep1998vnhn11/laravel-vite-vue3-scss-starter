import { fetchPlaylist, fetchStreaming } from '/@/api/mp3'
import { Playlist, Song } from '/@/container/src/mp3/types'
...
class Player {
    public _howler: Nullable<Howl> = null
    private _volumeBeforeMuted: number = 0
    private _isMuted: boolean = false
    private _state = reactive<{
        volume: number
        currentSong: Nullable<Song>
        currentPlaylist: Nullable<Playlist>
        queueItems: Song[]
        recentItems: Song[]
        repeat: any
        isShuffle: boolean
    ...
    looper() {
        if (this._howler && this.isPlaying && !this.isProgressBusy) {
            try {
                ...
                this.progress.value =
                    current && total ? (+current / total) * 100 : 0
            } catch {}
        }
        setTimeout(() => {
            this.looper()
    ...
    get currentSong(): Nullable<Song> {
        return this._state.currentSong
    }

    set currentSong(song: Nullable<Song>) {
        this.currentSongId = song?.encodeId || ''
        let index = this.queues.findIndex(
        ...
        song && this.recentItems.push(song)
        this._state.currentSong = song
    }

    get currentPlaylist(): Nullable<Playlist> {
        return this._state.currentPlaylist
    }

    set currentPlaylist(playlist: Nullable<Playlist>) {
        if (playlist) {
    ...
    handleNextSong() {
        this.currentSong = this.nextSong || null
        this.loadSong(true)
    }
    ...
    async loadPlaylist(playlist: Playlist) {
        const result: any = await fetchPlaylist(playlist.encodeId).catch(
            () => false
        )
        if (result && result.data) {
